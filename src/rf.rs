...
use crate::expanding::ExpandingExtractor;
...
impl RFPredictor {
    #[new]
    #[pyo3(signature = (model_path, sub_model_key=None, input_columns=None))]
    pub fn new(
        model_path: String,
        ...
        Python::with_gil(|py| {
            ...
            let (model, scaler, feature_names_list) = if loaded.is_instance_of::<PyDict>() {
                ...
                if dict.contains("scaler")? {
                    ...
                    return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>("Unknown dictionary model format"));
                }
            } else {
                return Err(PyErr::new::<pyo3::exceptions::PyTypeError, _>("Expected dictionary model (joblib)"));
            };
            ...
            let cols = input_columns.unwrap_or_else(|| {
                vec!["Torque (Nm)".to_string(), "Position (rad)".to_string(), "Velocity (rad/s)".to_string()]
            });
            ...
            for fname_obj in feature_names.iter() {
                ...
                for col_name in cols.iter() {
                    if fname.starts_with(col_name) && fname.len() > col_name.len() {
                        ...
                        let rust_feat_name = match feat_name {
                            "std" => "std_dev",
                            "max" => "max_value",
                            "min" => "min_value",
                            _ => feat_name,
                        };
                        unique_features.insert(rust_feat_name.to_string());
                        found = true;
                        break;
                    }
                }
                if !found {
                     return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!("Could not map feature name: {}", fname)));
                }
            }

            let sorted_unique_features: Vec<String> = unique_features.into_iter().collect();
            let extractor = ExpandingExtractor::new(sorted_unique_features.clone(), cols.len());
    ...
    pub fn predict(&mut self, batch: PyArrowType<RecordBatch>) -> PyResult<Option<f64>> {
        ...
        for (i, &(col_idx, feat_idx)) in self.feature_mapping.iter().enumerate() {
            let col = res_batch.column(feat_idx).as_any().downcast_ref::<Float32Array>().unwrap();
            let val = col.value(col_idx);
    ...
    pub fn reset(&mut self) {
        ...
        self.extractor = ExpandingExtractor::new(feature_strs, self.input_columns.len());
    }
}
